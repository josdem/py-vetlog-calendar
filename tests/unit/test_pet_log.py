...
def session_fixture():
    """Sets up an empty, clean in-memory database for every individual test run."""
...
def test_get_logs_by_date_range_success(session: Session):
    # Arrange: Use PetRepository and PetService instead of the standalone log ones
    repo = PetRepository(session)
    service = PetService(repo)
    
    # Seed mock medical logs
